    ...
    public static function deserialize($data, string $class, ?array $httpHeaders = null)
    {
        if (null === $data) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Aspose\BarCode\Model\\' . $data->{$discriminator};
            ...
            foreach ($instance::swaggerTypes() as $property => $type) {
                ...
                $key = lcfirst($instance::attributeMap()[$property]);
                if (!isset($propertySetter) || !isset($data->{$key})) {
                    continue;
                }

                $instance->$propertySetter(self::deserialize($data->{$key}, $type));
            }
